...
void inicializarLibro(int id, char * nombre, char * escritor, float costo, genero clase, int cuantidad){
	libro *libro_nuevo = (libro *)malloc(sizeof(libro));

	error(libro_nuevo);

	libro_nuevo -> identificador = id;

	strcpy(libro_nuevo -> titulo, nombre);
	strcpy(libro_nuevo -> autor, escritor);
	libro_nuevo -> precio = costo;
	libro_nuevo -> literario = clase;
	libro_nuevo -> cantidad = cuantidad;
}
...
void mostrarLibreria(libro *libros_a_imprimir){
	for(int o = 0; o < MAX_LIBROS; o++, libros_a_imprimir++)
		mostrarLibro(libros_a_imprimir);
}
